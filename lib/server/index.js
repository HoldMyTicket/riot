...
function getTagHtml(tag) {
  // Patch: ensure value property is reflected as attribute for SSR
  function syncValueAttr(node) {
    if (!node || !node.querySelectorAll) return
    // input (only type text, not checkbox/radio)
    Array.from(node.querySelectorAll('input')).forEach(function(el) {
      var type = el.getAttribute('type')
      if ((type === null || type === 'text') && el.value !== undefined) {
        el.setAttribute('value', el.value)
      }
    })
    // textarea
    Array.from(node.querySelectorAll('textarea')).forEach(function(el) {
      if (el.value !== undefined) el.textContent = el.value
    })
    // select
    Array.from(node.querySelectorAll('select')).forEach(function(el) {
      Array.from(el.options).forEach(function(opt) {
        if (opt.selected) opt.setAttribute('selected', 'selected')
      })
    })
  }
  syncValueAttr(tag.root)
  if (typeof sdom.serialize === 'function') {
    return sdom.serialize(tag.root)
  } else if (tag.root && tag.root.outerHTML) {
    return tag.root.outerHTML
  } else {
    return ''
  }
}
